...
assert(/tutor\.js\?v=157/.test(basecampHtml) && /basecamp\.js\?v=157/.test(basecampHtml), "Base Camp should cache-bust tutor/basecamp");
...
assert.strictEqual(weekById["chem-aboutme-disc"].canvas && weekById["chem-aboutme-disc"].canvas.status, "submitted");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.points, 1);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.score, null);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.submitted_at, "2026-08-21T10:58:00");
assert.strictEqual(weekById["chem-aboutme-disc"].school_status, "submitted");
...
assert.strictEqual((week.work || []).filter((w) => w.discrepancy === true).map((w) => w.id).join(","), "chem-about-me,chem-aboutme-disc", "About Me assignment + Get to Know You discussion are discrepancies");
assert.strictEqual((week.work || []).filter((w) => w.followup && typeof w.followup === "object").length, 10, "Parenting put a followup object on all 10 work rows");
...
assert(/submitted Aug 21 10:58am/.test(weekById["chem-aboutme-disc"].note || "") && /Get to Know You Slides/.test(weekById["chem-aboutme-disc"].note || "") && /0\/1/.test(weekById["chem-aboutme-disc"].note || ""), "chem discussion note says submitted 10:58 score still pending");
...
assert.strictEqual(week.as_of, "2026-08-21T14:15:00");
assert(Array.isArray(week.grades), "week.json has ParentVUE grades[]");
assert.strictEqual(week.grades.length, 2, "grades[] is exactly sociology + english-10");
assert.deepStrictEqual(week.grades.map((g) => g.classId), ["sociology", "english-10"], "grades[] rows are sociology then english-10");
assert(!week.grades.some((g) => g.classId === "chemistry" || /chem/i.test(String(g.class || ""))), "do not invent a Chemistry grades[] row");
...
assert.strictEqual((week.work || []).length, 10, "work[] stays the live 10-row set");
assert.strictEqual((week.notes || []).length, 15, "notes[] stays the live 15-row set");
...
assert(notesByKey["2026-08-19\tMorning check (late 7am)"], "kept the 8/19 late-7am morning check");
...
assert.deepStrictEqual((week.work || []).map((w) => w.id).sort(), [
  ...
  "web-11",
  "web-12"
], "work ids stay the live 10-row set — upsert chem-safety, do not slice-replace");
...
assert.strictEqual(weekById["chem-aboutme-disc"].status, "submitted");
assert.strictEqual(weekById["chem-aboutme-disc"].score, null);
...
assert.strictEqual(weekById["eng-names"].school_status, "submitted");
...
assert.strictEqual(weekById["chem-about-me"].discrepancy, true);
assert.strictEqual(weekById["chem-aboutme-disc"].school_status, "submitted");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.status, "submitted");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.points, 1);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.score, null);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.submitted_at, "2026-08-21T10:58:00");
...
assert.strictEqual(weekById["chem-aboutme-disc"].student_status.as_of, "2026-08-20T07:30:00");
assert.strictEqual(weekById["chem-aboutme-disc"].discrepancy, true);
assert.strictEqual(weekById["chem-aboutme-disc"].discrepancy_reason, "Live Canvas: submitted 8/21 10:58am, score still –/1 not yet posted. Prior observer row was late 0/1 submitted 8/19 3:33pm. Bennett told Orin 8/20 he emailed Pfeiff; teacher-email send not verified — do not mark follow-up sent.");
...
assert.strictEqual(weekById["web-11"].student_status, null);
assert.strictEqual(weekById["web-11"].discrepancy, false);
assert.strictEqual(weekById["web-12"].school_status, "open");
assert.strictEqual(weekById["web-12"].student_status, null);
assert.strictEqual(weekById["web-12"].discrepancy, false);
assert.strictEqual(weekById["eng-letter"].school_status, null);
...
assert(/ungraded \(\/5\) as of Fri 2:15pm/.test(weekById["eng-names"].note || ""), "name video still ungraded as of Fri 2:15pm");
assert(/NOW 10\/10 graded/.test(weekById["eng-comics"].note || "") && /No submitted_at on Canvas/.test(weekById["eng-comics"].note || ""), "comic note is now 10/10 graded with no submitted_at");
...
assert.strictEqual(discSt.score, "");
assert.strictEqual(discSt.late, false, "chem discussion is submitted pending, not Late");
assert.strictEqual(discSt.submitted, true, "chem discussion is submitted");
assert(discSt.needsYou, "chem discussion still needs you while score is pending");
assert.strictEqual(discSt.discrepancy, true, "chem discussion is still a discrepancy");
assert.strictEqual(discSt.studentSaid, "Already emailed the teacher about About Me.");
assert.strictEqual(discSt.schoolStatus, "submitted");
...
assert(needIds.indexOf("chem-aboutme-disc") >= 0 && needIds.indexOf("web-11") >= 0, "Needs you includes late chem discussion and web 1.1");
assert(needIds.indexOf("web-12") < 0, "Needs you does not include next-Monday Web 1.2 at 7:10 CT");
assert(/0 missing, 2 late, 1 due today/.test(Game.parentNeedsLine(week, proofNow)), "parent line matches Progress counts");
assert(Game.workIsDiscrepancy(weekById["chem-aboutme-disc"], proofNow), "chem discussion is a discrepancy");
...
assert.strictEqual(discFollow.due_by, "2026-08-21T14:40:00");
...
assert(/Copy email/.test(Game.followupCardHtml(weekById["chem-aboutme-disc"], progress.classes, proofNow)), "follow-up card is copyable");
assert(/Already emailed the teacher about About Me/.test(Game.followupCardHtml(weekById["chem-aboutme-disc"], progress.classes, proofNow)), "Bennett's said-claim shows on the card");
...
assert(/Yes — send 2 teacher emails/.test(followHtml), "after-school check calls out both chem emails");
...
assert(/About Me Slides \(assignment\)/.test(Game.followupStripHtml(week, proofNow) || "") && /Get to Know You Slides \(discussion\)/.test(Game.followupStripHtml(week, proofNow) || ""), "This Week strip names both chem discrepancies");
...
assert.strictEqual((week.events || []).length, 80, "merge kept live events[] and appended three Fillingham pass-offs");
...
assert(/theme\.css\?v=157/.test(weekHtml) && /week\.js\?v=157/.test(weekHtml) && /game\.js\?v=157/.test(weekHtml) && /telemetry\.js\?v=157/.test(weekHtml), "index should cache-bust css/js");
...
["index.html", "progress.html", "parent.html", "messages.html", "admin.html", "characters.html", "ask.html", "basecamp.html", "story.html", "egg.html", "refs.html"].forEach((file) => {
  const html = fs.readFileSync(path.join(root, file), "utf8");
  assert(!/\?v=156\b/.test(html), file + " should not still cache-bust as v=156");
  assert(/\?v=157/.test(html), file + " should cache-bust v=157");
...
["index.html", "progress.html", "parent.html", "messages.html", "admin.html", "characters.html", "ask.html", "basecamp.html", "story.html", "egg.html", "refs.html", "ptable.html"].forEach((file) => {
  const html = fs.readFileSync(path.join(root, file), "utf8");
  assert(/js\/update\.js\?v=157/.test(html), file + " loads the live-build checker");
...
assert(/progress\.js\?v=157/.test(progressHtml) && /theme\.css\?v=157/.test(progressHtml), "Progress should cache-bust css/js");
...
assert.strictEqual(Game.workPlanFor(plannedFam, "chem-aboutme-disc").text, "Finish the slides tonight after practice.");
assert(!Game.isBennettAsk(Game.workPlanFor(plannedFam, "chem-aboutme-disc")), "a plan is not an unanswered question");
const plannedAgain = Game.saveWorkPlan(plannedFam, weekById["chem-aboutme-disc"], "Ask Mr. X first period.");
assert.strictEqual((plannedAgain.notes || []).filter((n) => n.kind === "plan").length, 1, "saving a plan updates the same row");
assert.strictEqual(Game.workPlanFor(plannedAgain, "chem-aboutme-disc").text, "Ask Mr. X first period.");
...
assert(/build:\s*155/.test(fs.readFileSync(path.join(root, "js/build.js"), "utf8")), "BW_BUILD should be 155");
assert(/2026-08-24T17:28:00-05:00/.test(fs.readFileSync(path.join(root, "js/build.js"), "utf8")), "BW_BUILD modified is 2026-08-24T17:28:00-05:00");
...
assert(chemDisc && chemDisc.score == null, "Chem discussion score is still pending on the week feed");
