...
assert(adminHtml.includes("js/update.js") && /Cache-Control/.test(adminHtml) && /no-store/.test(adminHtml), "Admin HTML asks the browser not to keep a stale shell");
...
assert(/AbortController/.test(tutorJs) && /25000/.test(tutorJs), "tutor asks time out instead of hanging Send");
...
assert(/tutor\.js\?v=180/.test(basecampHtml) && /basecamp\.js\?v=180/.test(basecampHtml), "Base Camp should cache-bust tutor/basecamp");
...
assert(/multiple/.test(basecampHtml.match(/id="bc-upload"[^>]*>/)[0]), "Upload accepts more than one file");
assert(/Type here/.test(basecampHtml) && /Paste a screenshot/.test(basecampHtml), "composer says you can type and paste");
...
const sixAsStill = library.items.find((item) => item.id === "crew-six-as");
const sixAsClip = library.items.find((item) => item.id === "crew-six-as-clip");
const sixAsRun = library.items.find((item) => item.id === "crew-six-as-run");
assert(sixAsStill && sixAsStill.kind === "image" && sixAsStill.character === "crew", "Six A's still should be in library.json");
assert.strictEqual(sixAsStill.path, "img/library/crew-six-as.jpg", "Six A's still path");
assert(sixAsClip && sixAsClip.kind === "video" && sixAsClip.character === "crew", "Six A's clip should be in library.json");
assert.strictEqual(sixAsClip.path, "img/library/crew-six-as.mp4", "Six A's clip path");
assert.strictEqual(sixAsClip.poster, "img/library/crew-six-as.jpg", "Six A's clip poster is the still");
assert(sixAsRun && sixAsRun.kind === "video" && sixAsRun.character === "crew", "Six A's rush should be in library.json");
assert.strictEqual(sixAsRun.path, "img/library/crew-six-as-run.mp4", "Six A's rush path");
assert.strictEqual(sixAsRun.poster, "img/library/crew-six-as-run.jpg", "Six A's rush poster");
assert(fs.existsSync(path.join(root, "img/library/crew-six-as.jpg")), "Six A's still should already be on disk");
assert(fs.existsSync(path.join(root, "img/library/crew-six-as.mp4")), "Six A's clip should already be on disk");
assert(fs.existsSync(path.join(root, "img/library/crew-six-as-run.mp4")), "Six A's rush should already be on disk");
assert(fs.existsSync(path.join(root, "img/library/crew-six-as-run.jpg")), "Six A's rush poster should already be on disk");
assert(!sixAsStill.slot && !sixAsClip.slot, "Six A's still and 6s clip stay catalogue art");
assert.strictEqual(sixAsRun.slot, "content", "Six A's rush is gated until the login award");
...
assert.strictEqual(sixAsAch.rewardMedia, "chunky", "Six A's writeup plays Chunky like Meet Riff");
assert.deepStrictEqual(sixAsAch.rewardClips, ["crew-six-as-clip", "crew-six-as-run"], "Six A's trophy can replay both clips");
assert(/Preview as Bennett does not count/i.test(sixAsAch.how || ""), "Six A's how says Preview does not count");
...
assert(/six-as-classes/.test(parentHtml), "parent desk fallback should list Six A's in your classes");
assert(!/six-as-classes/.test(gameJs.slice(gameJs.indexOf("const PREVIEW_AWARD_IDS"), gameJs.indexOf("const PREVIEW_CHARACTER_IDS"))), "live Six A's is not a preview-only trophy");
...
assert(adminHtml.includes("img/library/crew-six-as.jpg") && adminHtml.includes("img/library/crew-six-as.mp4") && adminHtml.includes("img/library/crew-six-as-run.mp4"), "Admin file:// seed should include Six A's still and clips");
assert(/Paste the Downloads path in chat/.test(adminHtml), "Admin should say story clips ship from chat, not Choose files");
...
assert(!/help-chip/.test(hudPaint) && /messages-chip[\s\S]+parent-chip/.test(hudPaint), "Help is not a HUD chip; Messages still sits before Parent desk");
assert(/target="_blank"/.test(Game.helpLaunchHtml("index.html")) && /noopener/.test(Game.helpLaunchHtml("index.html")), "Help opens a new tab");
assert(!/target="_blank"/.test(Game.helpLaunchHtml("help.html")) && /help-launch on/.test(Game.helpLaunchHtml("help.html")), "Help page does not open another Help tab");
assert(/Snooze until/.test(Game.bennettHelpPageHtml()) && /Base Camp/.test(Game.bennettHelpPageHtml()) && /Paste a screenshot/.test(Game.bennettHelpPageHtml()), "Bennett Help covers snooze, Base Camp, and paste");
assert(/help-toc/.test(Game.bennettHelpPageHtml()) && /help-search/.test(Game.bennettHelpPageHtml()), "Help has a table of contents and search");
assert(Game.filterBennettHelp("snooze").some((row) => row.id === "follow-up"), "Help search finds snooze");
assert(Game.filterBennettHelp("6 A's").some((row) => row.id === "trophy-room" && /pedestal/.test(row.body)), "Help says 6 A's stays on the pedestal");
assert.strictEqual(Game.filterBennettHelp("zzzz-nope").length, 0, "Help search can miss");
assert(/<mark>/i.test(Game.markHelpMatch("Snooze until", "snooze")), "Help search marks hits");
...
assert(!/id="bennett-help"/.test(fs.readFileSync(path.join(root, "index.html"), "utf8")) && !/renderBennettHelp/.test(fs.readFileSync(path.join(root, "js/week.js"), "utf8")), "This Week does not dump Help copy");
assert(fs.existsSync(path.join(root, "help.html")) && /bennett-help-page/.test(fs.readFileSync(path.join(root, "help.html"), "utf8")) && /help-desk/.test(fs.readFileSync(path.join(root, "help.html"), "utf8")), "Help is its own page");
...
(function assertBennettStoryIsFindable() {
  ...
  const keys = ["bw-session", "bw-site-view", "bw-telemetry", "bw-story-unlock", "bw-login-days", "bw-character-unlocks"];
  const prev = {};
  keys.forEach((key) => { prev[key] = localStorage.getItem(key); localStorage.removeItem(key); });
  const qsPrev = document.querySelectorAll;
  try {
    assert.strictEqual(typeof Game.storyAvailable, "function", "storyAvailable is exported");
    assert.strictEqual(typeof Game.shouldRecordBennettLogin, "function", "shouldRecordBennettLogin is exported");
    assert(/storyAvailable/.test(storyJs) && !/comicUnlocked\(roster\)/.test(storyJs), "Story page opens for Bennett without a teammate gate");
    assert(!/Unlock three teammates/.test(storyHtml), "Story gate no longer tells Bennett to unlock three teammates");
    assert(/function storyAvailable/.test(gameSrc) && /paintStoryChip/.test(gameSrc), "HUD Story chip uses storyAvailable");
    assert(Game.filterBennettHelp("story").some((row) => row.id === "story" && /one new page/i.test(row.body) && /pops up on This Week/i.test(row.body) && /signed in/i.test(row.body) && !/Coming soon/.test(row.body) && !/three teammates/i.test(row.body)), "Help says Story is on the bar when Bennett is signed in");
    assert(!Game.comicUnlocked(roster), "empty locker does not meet comicStartsAfter");
    assert(!Game.storyAvailable(roster), "Me view without teammates still hides Story");
    localStorage.setItem("bw-session", JSON.stringify({ user: "bennett", at: "2026-09-15T13:00:00.000Z" }));
    assert(Game.storyAvailable(roster), "Bennett login opens Story with no teammates unlocked");
    assert(Game.shouldRecordBennettLogin(), "Bennett login records a story day");
    const chip = { hidden: true, id: "story-chip" };
    document.querySelectorAll = (sel) => (sel && String(sel).indexOf("story-chip") >= 0 ? [chip] : []);
    Game.paintStoryChip(roster);
    assert.strictEqual(chip.hidden, false, "paintStoryChip unhides Story for Bennett login");
    localStorage.removeItem("bw-session");
    chip.hidden = true;
    Game.paintStoryChip(roster);
    assert.strictEqual(chip.hidden, true, "paintStoryChip hides Story again when Bennett is signed out");
    document.querySelectorAll = qsPrev;
    localStorage.setItem("bw-session", JSON.stringify({ user: "orin", at: "2026-09-15T13:00:00.000Z" }));
    localStorage.setItem("bw-site-view", "bennett");
    assert(Game.storyAvailable(roster), "Preview as Bennett still shows Story");
    assert(!Game.shouldRecordBennettLogin(), "Orin preview as Bennett does not record a story day");
    localStorage.removeItem("bw-session");
    localStorage.setItem("bw-telemetry", JSON.stringify({ role: "bennett" }));
    localStorage.setItem("bw-site-view", "me");
    assert(Game.storyAvailable(roster), "Bennett device shows Story even if the view was left on Me");
    assert(Game.shouldRecordBennettLogin(), "Bennett device without a parent session records login days");
    localStorage.setItem("bw-session", JSON.stringify({ user: "orin", at: "2026-09-15T13:00:00.000Z" }));
    localStorage.setItem("bw-site-view", "bennett");
    assert(!Game.shouldRecordBennettLogin(), "Orin on a Bennett-tagged phone does not advance the strip");
    localStorage.removeItem("bw-session");
    localStorage.removeItem("bw-story-unlock");
    localStorage.setItem("bw-session", JSON.stringify({ user: "bennett", at: "2026-09-15T13:00:00.000Z" }));
    assert.strictEqual(Game.recordStoryDay("2026-09-15").reached, 1, "Bennett login day 1 unlocks page 1");
    assert.strictEqual(Game.visibleStoryPages(storyJson).map((p) => p.id).join(","), "pack");
    assert.strictEqual(Game.recordStoryDay("2026-09-16").reached, 2, "Bennett login day 2 unlocks page 2");
    assert.strictEqual(Game.visibleStoryPages(storyJson, { preview: true }).length, 8, "parent preview still sees the full strip");
  } finally {
    document.querySelectorAll = qsPrev;
    keys.forEach((key) => {
      if (prev[key] == null) localStorage.removeItem(key);
      else localStorage.setItem(key, prev[key]);
    });
  }
})();
...
assert(!/bc-calc-key wide/.test(basecampHtml) && !/\.bc-calc-key\.wide/.test(bcThemeCss), "180− does not span two columns and shove the digits");
const calcKeys = Array.from(basecampHtml.matchAll(/data-calc="([^"]+)"/g)).map((m) => m[1]);
assert.deepStrictEqual(calcKeys.slice(8, 16), ["7", "8", "9", "÷", "4", "5", "6", "×"], "digit pad is 7 8 9 ÷ / 4 5 6 ×");
...
assert(/help-launch/.test(fs.readFileSync(path.join(root, "ptable.html"), "utf8")) && /target="_blank"/.test(fs.readFileSync(path.join(root, "ptable.html"), "utf8")), "periodic table has Help in a new tab");
...
assert(/disableWorker/.test(basecampJs) && /password locked/.test(basecampJs), "PDF reader retries without a worker and names a locked file");
assert(/onComposerPaste/.test(basecampJs) && /clipboardData/.test(basecampJs) && /onComposerDrop/.test(basecampJs), "composer paste and drop attach screenshots and PDFs");
assert(/addEventListener\("paste"/.test(basecampJs), "paste is bound on the composer");
const sendFn = basecampJs.slice(basecampJs.indexOf("async function send"), basecampJs.indexOf("const INTRO_SRC"));
assert(/finally/.test(sendFn) && /sending = false/.test(sendFn), "a failed tutor call cannot leave Send stuck");
...
["academic-intervention", "strength", "geometry"].forEach((id) => {
...
assert.strictEqual(weekById["chem-aboutme-disc"].title, "Chemistry: Get to Know You Slides (discussion)");
assert.strictEqual(weekById["chem-aboutme-disc"].due, "2026-08-21T23:59:00");
assert.strictEqual(weekById["chem-aboutme-disc"].classId, "chemistry");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas && weekById["chem-aboutme-disc"].canvas.status, "graded");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.points, 1);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.score, 1);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.submitted_at, "2026-08-21T10:58:00");
assert.strictEqual(weekById["chem-aboutme-disc"].school_status, "graded");
assert.strictEqual(weekById["chem-aboutme-disc"].student_status && weekById["chem-aboutme-disc"].student_status.said, "Already emailed the teacher about About Me.");
...
assert.strictEqual(weekById["chem-aboutme-disc"].discrepancy, false);
assert(weekById["chem-aboutme-disc"].discrepancy_reason, "Parenting left a discrepancy_reason on the chem discussion");
...
assert.strictEqual((week.work || []).filter((w) => w.followup && typeof w.followup === "object").length, 25, "Parenting put a followup object on all work rows");
...
assert.strictEqual(weekById["chem-about-me"].followup && weekById["chem-about-me"].followup.email_draft, null, "do not write an email_draft into week.json");
assert.strictEqual(weekById["chem-about-me"].followup.email_sent, false, "do not invent that an email was sent");
assert(/Submitted Aug 21 10:58am/.test(weekById["chem-aboutme-disc"].note || "") && /GRADED 1\/1/.test(weekById["chem-aboutme-disc"].note || ""), "chem discussion note says graded 1/1 after the 10:58 submit");
assert(!/\bMISSED\b/.test(weekById["chem-aboutme-disc"].note || ""), "chem discussion is no longer MISSED");
...
assert.strictEqual(weekById["eng-names"].canvas && weekById["eng-names"].canvas.status, "graded");
...
assert(/GRADED Sun 8\/23/.test(weekById["eng-names"].note || ""), "name video note says graded");
assert.strictEqual(weekById["eng-names"].canvas.score, 5);
...
assert.strictEqual((week.work || []).filter((w) => /English 10/.test(w.title || "")).length, 11, "English keeps names+comics+letter+notebook plus choice book, AAB, 10.1, and Formative On-Demand");
assert.strictEqual(week.as_of, "2026-08-31T15:16:00");
assert(Array.isArray(week.grades), "week.json has ParentVUE grades[]");
assert.strictEqual(week.grades.length, 7, "grades[] is the seven ParentVUE classes with marks");
assert.deepStrictEqual(week.grades.map((g) => g.classId), ["sociology", "english-10", "chemistry", "web-design", "geometry", "band", "strength"], "grades[] rows follow the ParentVUE pull");
...
const webGrade = week.grades.find((g) => g.classId === "web-design");
assert(socGrade && socGrade.letter === "A" && socGrade.percent === 97 && socGrade.as_of === "2026-08-31", "Sociology ParentVUE is A 97% as of 8/31");
assert(engGrade && engGrade.letter === "A+" && engGrade.percent === 98.33 && engGrade.as_of === "2026-08-31", "English ParentVUE is A+ 98.33% as of 8/31");
assert(chemGrade && chemGrade.letter === "B" && chemGrade.percent === 85.7 && chemGrade.as_of === "2026-08-31", "Chemistry ParentVUE is B 85.7% as of 8/31");
assert(webGrade && webGrade.letter === "A" && webGrade.percent === 96 && webGrade.as_of === "2026-08-31", "Web Design ParentVUE is A 96% as of 8/31");
assert.strictEqual(Game.gradesFromWeek({}).length, 0, "gradesFromWeek is empty when grades[] is missing");
assert.deepStrictEqual([...Game.gradesFromWeek(week)].map((g) => g.classId), ["sociology", "english-10", "chemistry", "web-design", "geometry", "band", "strength"]);
...
assert.strictEqual(Game.gradeForClass(week, "chemistry").letter, "B");
const socPill = Game.gradePillModel(Game.gradeForClass(week, "sociology"));
assert.strictEqual(socPill.display, "A 97%");
assert.strictEqual(socPill.detail, "Last update 8/31 (was 8/30). NEW: 1.3 Reading due 9/1 Not Due 16 pts. CSI 1.1 30/30; 1.2 12/12; 1.1 14/14; Scavenger Hunt 20/22. Room 315. · ParentVUE 8/31");
const engPill = Game.gradePillModel(Game.gradeForClass(week, "english-10"));
assert.strictEqual(engPill.display, "A+ 98.33%");
assert.strictEqual(engPill.detail, "Last update 8/30. Choice Book 5/5; Formative 9.25/10; 10.1 Skills 5/5; letter 10/10; comic 10/10; names 5/5. No AAB Reading Check #1 row in ParentVUE. Room 816. · ParentVUE 8/31");
assert.strictEqual(Game.gradePillModel({ classId: "chemistry", source: "ParentVUE" }), null);
assert(/B\s*85\.7%/.test(Game.gradePillHtml(week, "chemistry")));
assert(/class="grade-pill"/.test(Game.gradePillHtml(week, "sociology")) && /A\s*97%/.test(Game.gradePillHtml(week, "sociology")) && /grade-detail/.test(Game.gradePillHtml(week, "sociology")));
assert(/class="grade-pill"/.test(Game.gradePillHtml(week, "english-10")) && /A\+\s*98\.33%/.test(Game.gradePillHtml(week, "english-10")));
assert.strictEqual((week.work || []).length, 27, "work[] stays the live set");
assert.strictEqual((week.notes || []).length, 41, "notes[] stays the live set");
...
assert(/Chem discussion now submitted Aug 19 3:33pm/.test(notesByTitle["Morning check (~7:10am)"].text || ""), "morning check mentions late chem discussion");
assert(notesByTitle["Discrepancy layer (Plaud 7:30am)"], "discrepancy-layer note is present");
assert.strictEqual(notesByTitle["Discrepancy layer (Plaud 7:30am)"].date, "2026-08-20");
assert(/follow-up send not verified/.test(notesByTitle["Discrepancy layer (Plaud 7:30am)"].text || ""), "discrepancy note says follow-up send is not verified");
...
assert(notesByKey["2026-08-24\tAfter-school check (~5:15pm CT) — missed 2:10 run"], "kept the 8/24 after-school check");
assert(/Chem NEW grades/.test(notesByKey["2026-08-24\tAfter-school check (~5:15pm CT) — missed 2:10 run"].text || ""), "8/24 check mentions new Chem grades");
assert(notesByKey["2026-08-21\tAfter-school check (~2:15pm)"], "appended the 8/21 2:15pm after-school check");
assert(/Safety Exam/.test(notesByKey["2026-08-21\tAfter-school check (~2:15pm)"].text || "") && /10\/10/.test(notesByKey["2026-08-21\tAfter-school check (~2:15pm)"].text || ""), "2:15pm check mentions comic 10/10 and chem safety");
assert(notesByKey["2026-08-21\tEnglish agenda"], "appended the 8/21 English agenda note");
assert(notesByKey["2026-08-28\tDWT on-demand"], "DWT is a note only");
assert(!(week.events || []).some((e) => /DWT/i.test(e.title || "") || /dwt/i.test(e.id || "")), "do not invent a DWT event");
...
assert.deepStrictEqual((week.work || []).map((w) => w.id).sort(), [
  ...
  "eng-aab-p40",
  "eng-aab-p82",
  "eng-aab-reading-check-1",
  "eng-choicebook",
  "eng-comics",
  "eng-formative-ondemand",
  "eng-letter",
  "eng-names",
  "eng-notebook",
  "web-11",
...
assert.strictEqual(weekById["chem-safety"].status, "graded");
assert.strictEqual(weekById["chem-safety"].submitted_at, "2026-08-21T11:15:00");
assert.strictEqual(weekById["chem-safety"].discrepancy, false);
assert.strictEqual(weekById["chem-safety"].followup && weekById["chem-safety"].followup.email_draft, null);
assert.strictEqual(weekById["chem-safety"].followup.email_sent, false);
...
assert.strictEqual(weekById["chem-penny"].status, "graded");
assert.strictEqual(weekById["chem-penny"].points, 1);
assert.strictEqual(weekById["chem-penny"].score, "1/1");
assert.strictEqual(weekById["chem-penny"].late, false);
assert.strictEqual(weekById["chem-penny"].school_status, "graded");
...
assert.strictEqual(weekById["chem-aboutme-disc"].status, "graded");
assert.strictEqual(weekById["chem-aboutme-disc"].score, "1/1");
assert.strictEqual(weekById["chem-aboutme-disc"].late, false);
assert.strictEqual(weekById["chem-aboutme-disc"].submitted_at, "2026-08-21T10:58:00");
assert.strictEqual(weekById["eng-names"].school_status, "graded");
assert.strictEqual(weekById["eng-names"].student_status, null);
assert.strictEqual(weekById["eng-names"].discrepancy, false);
assert.strictEqual(weekById["eng-names"].followup.due_by, null);
assert.strictEqual(weekById["eng-names"].followup.email_draft, null);
assert.strictEqual(weekById["eng-names"].followup.email_sent, false);
assert.strictEqual(weekById["eng-comics"].school_status, "graded");
assert.strictEqual(weekById["eng-comics"].status, "graded");
assert.strictEqual(weekById["eng-comics"].score, "10/10");
assert.strictEqual(weekById["eng-comics"].student_status, null);
assert.strictEqual(weekById["eng-comics"].submitted_at, null);
assert.strictEqual(weekById["eng-comics"].graded_at, "2026-08-21T10:38:00");
assert.strictEqual(weekById["eng-comics"].discrepancy, false);
assert.strictEqual(weekById["eng-comics"].followup.email_sent, false);
assert(Array.isArray(weekById["eng-comics"].canvas && weekById["eng-comics"].canvas.rubric), "comic rubric stays with discrepancy layer");
assert.strictEqual(weekById["chem-about-me"].school_status, "open");
assert.strictEqual(weekById["chem-about-me"].student_status && weekById["chem-about-me"].student_status.said, "Posted Google Slides link as a comment");
assert.strictEqual(weekById["chem-about-me"].discrepancy, true);
assert.strictEqual(weekById["chem-aboutme-disc"].school_status, "graded");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.status, "graded");
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.points, 1);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.score, 1);
assert.strictEqual(weekById["chem-aboutme-disc"].canvas.submitted_at, "2026-08-21T10:58:00");
assert.strictEqual(weekById["chem-aboutme-disc"].student_status.said, "Already emailed the teacher about About Me.");
assert.strictEqual(weekById["chem-aboutme-disc"].student_status.source, "Plaud with Orin 2026-08-20 ~7:30am");
assert.strictEqual(weekById["chem-aboutme-disc"].student_status.as_of, "2026-08-20T07:30:00");
assert.strictEqual(weekById["chem-aboutme-disc"].discrepancy, false);
assert(weekById["chem-aboutme-disc"].discrepancy_reason, "Parenting left a discrepancy_reason on the chem discussion");
...
assert.strictEqual(weekById["eng-letter"].student_status, null);
assert.strictEqual(weekById["eng-letter"].submitted_at, null);
assert.strictEqual(weekById["eng-letter"].discrepancy, false);
assert.strictEqual(weekById["eng-notebook"].school_status, "open");
assert.strictEqual(weekById["eng-notebook"].student_status, null);
assert.strictEqual(weekById["eng-notebook"].submitted_at, null);
assert.strictEqual(weekById["eng-notebook"].discrepancy, false);
assert(!(week.work || []).some((w) => w.followup && w.followup.email_sent === true), "do not invent email sent");
...
assert.strictEqual(discSt.score, "1/1");
assert.strictEqual(discSt.late, false, "chem discussion is graded, not Late");
assert.strictEqual(discSt.submitted, true, "chem discussion is submitted");
assert.strictEqual(discSt.graded, true, "chem discussion is graded");
assert.strictEqual(discSt.discrepancy, false, "graded chem discussion is closed follow-up");
assert.strictEqual(discSt.studentSaid, "Already emailed the teacher about About Me.");
assert.strictEqual(discSt.schoolStatus, "graded");
...
assert(Game.workIsDiscrepancy({
  id: "web-11-claim",
  title: weekById["web-11"].title,
  school_status: "late",
  student_status: "done",
  source: "canvas"
}, proofNow), "student-done + school late/unlogged is a discrepancy when the flag is omitted");
assert(!Game.workIsDiscrepancy(Object.assign({}, weekById["chem-aboutme-disc"], { discrepancy: false }), proofNow), "explicit discrepancy false wins");
assert.strictEqual(Game.teacherFromNote(weekById["chem-aboutme-disc"].note), "Pfeiff", "teacher comes from the note, not a guessed inbox");
const discFollow = Game.workFollowup(weekById["chem-aboutme-disc"], [], proofNow);
assert.strictEqual(discFollow.due_by, "");
assert.strictEqual(discFollow.email_sent, false, "follow-up helper does not invent email_sent");
assert(/Hey Mr\.\/Ms\. Pfeiff/.test(discFollow.email_draft), "null email_draft generates a Pfeiff email");
assert(/Get to Know You Slides \(discussion\)/.test(discFollow.email_draft) && /August 21/.test(discFollow.email_draft), "generated email has assignment and turn-in date");
assert(/not updated in Canvas/.test(discFollow.email_draft), "generated email uses school_status");
assert(/Copy email/.test(Game.followupCardHtml(weekById["chem-about-me"], progress.classes, proofNow)), "follow-up card is copyable");
assert(/Posted Google Slides link as a comment/.test(Game.followupCardHtml(weekById["chem-about-me"], progress.classes, proofNow)), "Bennett's said-claim shows on the card");
const followHtml = Game.followupSectionHtml(week, progress.classes, proofNow, { mode: "after-school", page: "progress.html" });
assert(/Needs follow-up/.test(followHtml) && /do I need to send any emails/.test(followHtml), "check-in asks if any emails need sending");
assert(/Yes — send 1 teacher email/.test(followHtml), "after-school check calls out the remaining chem email");
assert(/School/.test(followHtml) && /Bennett/.test(followHtml) && /Posted Google Slides link as a comment/.test(followHtml), "follow-up shows school vs Bennett");
...
assert(/About Me Slides \(assignment\)/.test(Game.followupStripHtml(week, proofNow) || "") && !/Get to Know You Slides \(discussion\)/.test(Game.followupStripHtml(week, proofNow) || ""), "This Week strip names the open chem discrepancy, not the graded discussion");
assert(/Snooze until/.test(Game.followupCardHtml(weekById["chem-about-me"], progress.classes, proofNow)) && /data-followup-snooze/.test(Game.followupCardHtml(weekById["chem-about-me"], progress.classes, proofNow)), "follow-up card has a snooze-until date picker");
const geoPacket = {
  id: "geo-packet",
  title: "Geo Lesson 1.2: Points, Lines, and Planes",
  school_status: "open",
  student_status: "done",
  source: "canvas"
};
assert(Game.workIsDiscrepancy(geoPacket, proofNow), "finished packet stays in follow-up until snoozed or school logs it");
assert.strictEqual(Game.emailsToSend({ work: [Object.assign({}, geoPacket, { followupSnoozeUntil: "2026-08-28" })] }, proofNow).length, 0, "snooze-until a future date hides the packet");
assert.strictEqual(Game.emailsToSend({ work: [Object.assign({}, geoPacket, { followupSnoozeUntil: "2026-08-20" })] }, proofNow).length, 1, "snooze-until today puts it back");
assert(!Game.workIsDiscrepancy(Object.assign({}, geoPacket, { discrepancy: true, school_status: "graded", score: "10/10" }), proofNow), "graded work leaves Needs follow-up even with discrepancy true");
assert(!Game.workIsDiscrepancy(Object.assign({}, geoPacket, { discrepancy: true, school_status: "graded", score: "10/10", followupSnoozeUntil: "2026-09-01" }), proofNow), "closed graded work stays out even if still snoozed");
assert(!Game.workIsDiscrepancy(Object.assign({}, geoPacket, { school_status: "submitted", submitted_at: "2026-08-27T09:00:00" }), proofNow), "school-logged submit is no longer inferred follow-up");
const snoozeViewBefore = Game.siteView();
const snoozeFamBefore = localStorage.getItem("bw-family");
Game.setSiteView("bennett");
assert(!Game.progressCanMutate(), "Bennett cannot mutate Progress catalog");
assert(/data-followup-snooze/.test(Game.followupCardHtml(weekById["chem-about-me"], progress.classes, proofNow)), "Bennett still gets the snooze date picker");
Game.setFollowupSnooze("chem-about-me", "2026-08-28");
assert.strictEqual(Game.followupSnoozeUntil({ id: "chem-about-me" }), "2026-08-28", "Bennett snooze persists on the family overlay");
assert(!Game.workIsDiscrepancy(weekById["chem-about-me"], proofNow), "snoozed assignment leaves Needs follow-up");
assert(Game.followupIsSnoozed(weekById["chem-about-me"], proofNow), "overlay snooze hides until the picked date");
Game.setFollowupSnooze("chem-about-me", "");
assert(Game.workIsDiscrepancy(weekById["chem-about-me"], proofNow), "clearing snooze puts the assignment back");
...
assert(!Game.workFeedStatus(weekById["chem-about-me"], proofNow).needsYou, "Done work leaves Needs you");
assert(Game.needsYouWork(week, proofNow).every((w) => w.id !== "chem-about-me"), "Needs you drops work Bennett marked Done");
assert(/data-needs-done/.test(gameJs), "Needs you rows have a Done control");
if (!chemDoneBefore) Game.touchWork("chem-about-me", "done");
assert(/data-needs-done="chem-about-me"/.test(Game.needsYouListHtml(week, proofNow)), "Needs you shows Done next to Plan and Edit");
...
assert.strictEqual((week.events || []).length, 89, "merge kept live events[] including Fillingham pass-offs and boosters");
...
assert(!chemTue.due.some((w) => w.id === "chem-aboutme-disc"), "Get to Know You discussion is due Friday, not Tuesday");
const chemFri = Game.itemsForClassOnDay(week, "chemistry", new Date(2026, 7, 21));
assert(chemFri.due.some((w) => w.id === "chem-aboutme-disc"), "Fri 8/21 Chem card shows Get to Know You Slides discussion");
assert(chemFri.due.some((w) => w.id === "chem-penny"), "Fri 8/21 Chem card shows penny demo");
...
assert.strictEqual(Game.classAttentionCount({ id: "english-10" }, week, weekDays, proofNow), 0, "graded English work is not an attention badge");
...
assert.strictEqual(Game.classDueCount(english, week), 11, "English due count matches week.json");
assert.strictEqual(Game.classDueCount(byId.chemistry, week), 9, "Chemistry due count matches week.json");
assert.strictEqual(Game.classDueCount(byId["web-design"], week), 7, "Web Design due count matches week.json");
...
assert(Game.libraryVisualItems(seededLib).some((item) => item.id === "ace-frog" && item.kind === "video"), "catalogue includes Frog Serve");
assert(Game.libraryVisualItems(seededLib).some((item) => item.id === "crew-adventure" && item.kind === "video"), "catalogue includes crew adventure clip");
assert(Game.libraryVisualItems(seededLib).some((item) => item.id === "crew-six-as" && item.kind === "image"), "catalogue includes Six A's still");
assert(Game.libraryVisualItems(seededLib).some((item) => item.id === "crew-six-as-clip" && item.kind === "video"), "catalogue includes Six A's clip");
assert(Game.libraryVisualItems(seededLib).some((item) => item.id === "crew-six-as-run" && item.kind === "video"), "catalogue includes Six A's rush");
assert(Game.libraryVisualItems(seededLib).some((item) => item.id === "ace-clip"), "catalogue includes Ace locker clip");
assert(!Game.libraryVisualItems(seededLib).some((item) => item.id === "tablesloud"), "catalogue is clips and stills, not Fun audio");
assert(Game.libraryVisualItems(Game.normalizeLibrary({
  items: [{ id: "fun-clip", label: "Fun clip", path: "img/library/ace-frog.mp4", kind: "video", character: "fun" }]
})).some((item) => item.id === "fun-clip"), "Fun-tagged videos still appear in the All catalogue");
assert(/id="library-catalogue"/.test(adminJs) && /libraryVisualItems/.test(adminJs), "Admin All shows one clips-and-stills catalogue");
assert(/id="crew-lib-cats"/.test(parentHtml) && /data-crew-lib="all"/.test(parentHtml), "Parent Crew has an All catalogue filter");
assert(/id="char-library"/.test(parentHtml) && !/<section class="char-library" id="char-library" hidden>/.test(parentHtml), "Parent Crew library is visible without picking a teammate");
...
assert(Game.defaultLibrary().items.some((item) => item.id === "crew-six-as-clip" && item.path === "img/library/crew-six-as.mp4" && item.character === "crew"), "file:// default library includes Six A's clip");
assert(Game.defaultLibrary().items.some((item) => item.id === "crew-six-as-run" && item.slot === "content"), "file:// default library gates Six A's rush");
assert(Game.canPlayLibraryItem(Game.libraryItem(seededLib, "crew-six-as-clip")), "Six A's clip stays playable in the catalogue");
assert(!Game.canPlayLibraryItem(Game.libraryItem(seededLib, "crew-six-as-run")), "Six A's rush stays locked until awarded");
...
const parentJsTabs = fs.readFileSync(path.join(root, "js/parent.js"), "utf8");
assert(/bindParentTabs/.test(parentJsTabs) && /data-parent-tab/.test(parentJsTabs), "Parent desk tabs switch panels");
assert(/const PARENT_TABS = \["awards", "crew", "sounds", "notes", "daily", "classes", "story", "pack"\]/.test(parentJsTabs), "Parent tab ids are defined so Awards can hide Crew");
assert.strictEqual((parentHtml.match(/data-parent-panel="awards"/g) || []).length, 1, "Awards is one panel");
const awardsBlock = parentHtml.slice(parentHtml.indexOf('data-parent-panel="awards"'), parentHtml.indexOf('data-parent-panel="pack"'));
assert(/id="list"/.test(awardsBlock) && /id="add"/.test(awardsBlock) && /id="award-find"/.test(awardsBlock), "Awards tab owns Schedule reward, Find, and the rewards list");
assert(/mergeAchievementUnlocks\(livePack/.test(parentJsTabs), "desk sync keeps shipped awards in a mom draft");
const draftSansSix = Game.mergeAchievementUnlocks({ achievements: [{ id: "straight-as-3w", title: "Straight A's" }] }, achievements);
assert(draftSansSix.achievements.some((a) => a.id === "six-as-classes"), "shipped Six A's lands in a mom draft that lacked it");
...
assert(/Next Bennett login/.test(parentJs), "Parent desk has a Next Bennett login earn chip");
assert(/bennett_login/.test(parentJs), "Parent desk Save can persist a next-login unlock");
...
assert(/theme\.css\?v=180/.test(weekHtml) && /week\.js\?v=180/.test(weekHtml) && /game\.js\?v=180/.test(weekHtml) && /telemetry\.js\?v=180/.test(weekHtml), "index should cache-bust css/js");
...
assert(/html:has\(body\.week-page:not\(\.in-treehouse\)\)/.test(weekMobileCss) && /overflow-y:\s*auto/.test(weekMobileCss), "week-page mobile CSS allows vertical page overflow");
const weekMobileLock = weekMobileCss.match(/html:has\(body\.week-page:not\(\.in-treehouse\)\)\s*\{[^}]+\}/);
...
assert(/html:has\(body\.week-page:not\(\.in-treehouse\)\)/.test(weekDesktopCss) && /overflow-y:\s*auto/.test(weekDesktopCss), "desktop week-page html can scroll");
assert(/body\.week-page:not\(\.in-treehouse\)[\s\S]{0,220}display:\s*block/.test(weekDesktopCss), "This Week is document flow, not a flex viewport lock");
assert(/body\.week-page:not\(\.in-treehouse\)[\s\S]{0,220}overflow:\s*visible/.test(weekDesktopCss), "This Week body does not trap page scroll");
...
assert(/body\.week-page \.card-scroll[\s\S]{0,120}overflow:\s*visible/.test(weekDesktopCss) && /body\.week-page \.card::after[\s\S]{0,80}display:\s*none/.test(weekDesktopCss), "week-page desktop card-scroll is visible and fade is off");
...
["index.html", "progress.html", "parent.html", "messages.html", "admin.html", "characters.html", "ask.html", "basecamp.html", "story.html", "egg.html", "refs.html", "help.html"].forEach((file) => {
  ...
  assert(/\?v=180/.test(html), file + " should cache-bust v=180");
  ...
  assert(/help-launch/.test(bar) && !/help-chip/.test(bar), file + " pins Help at the top right, not in the chip row");
  if (file === "help.html") {
    assert(!/target="_blank"/.test(bar), "Help page Help button stays in this tab");
  } else {
    assert(/target="_blank"/.test(bar) && /noopener/.test(bar), file + " Help opens a new tab");
  }
...
["index.html", "progress.html", "parent.html", "messages.html", "admin.html", "characters.html", "ask.html", "basecamp.html", "story.html", "egg.html", "refs.html", "ptable.html", "help.html"].forEach((file) => {
  const html = fs.readFileSync(path.join(root, file), "utf8");
  assert(/js\/update\.js\?v=180/.test(html), file + " loads the live-build checker");
...
assert(/js\/build\.js\?t=/.test(updateJs) && /cache:\s*"no-store"/.test(updateJs), "live-build checker fetches build.js without cache");
assert(/location\.replace/.test(updateJs) && /BW_BUILD/.test(updateJs), "stale page reloads onto the live build");
assert(/serviceWorker\.register\("sw\.js"\)/.test(updateJs), "http(s) visits register a network-first worker");
const swJs = fs.readFileSync(path.join(root, "sw.js"), "utf8");
assert(/skipWaiting/.test(swJs) && /clients\.claim/.test(swJs) && /cache:\s*"no-store"/.test(swJs), "service worker fetches the live site, not a cached copy");
assert(/t=" \+ Date\.now\(\)/.test(gameJs) && /cache:\s*"no-store"/.test(gameJs), "week/progress JSON fetches are cache-busted");
...
assert(/\.hud-bar[\s\S]{0,280}grid-template-columns:\s*max-content minmax\(0,\s*1fr\) max-content/.test(themeCss), "HUD is bananas, nav, and Help");
...
assert(/button:disabled \{ cursor:\s*default/.test(themeCss) && /usage-range-btn\.on[\s\S]{0,80}cursor:\s*default/.test(themeCss), "only actionable controls show a hand");
assert(/minmax\(360px,\s*2fr\)/.test(themeCss), "Grades pane is twice as tall");
...
assert(/progress\.js\?v=180/.test(progressHtml) && /theme\.css\?v=180/.test(progressHtml), "Progress should cache-bust css/js");
...
localStorage.setItem("bw-needs-you-collapsed", JSON.stringify(true));
window.location.pathname = "/progress.html";
assert(!Game.needsYouCollapsed(), "Progress Needs you defaults to shown even if This Week was hidden");
assert(/needs-you-toggle-hint">Hide/.test(Game.needsYouSectionHtml(week, proofNow, { empty: true })), "Progress Needs you default control is Hide");
window.location.pathname = "/index.html";
assert(Game.needsYouCollapsed(), "This Week still remembers Hide");
localStorage.removeItem("bw-needs-you-collapsed");
assert(!Game.needsYouCollapsed(), "Needs you is shown when nothing is stored");
...
assert(/bw-followup-changed/.test(weekJs) && /bw-followup-changed/.test(progressJs) && /bindFollowupSnooze/.test(gameJs), "This Week and Progress refresh after a snooze");
...
const plannedFam = Game.saveWorkPlan(Game.emptyFamily(), weekById["chem-about-me"], "Finish the slides tonight after practice.");
assert.strictEqual(Game.getBananas(), bananasBeforePlan, "writing a plan does not pay bananas");
assert.strictEqual(Game.workPlanFor(plannedFam, "chem-about-me").text, "Finish the slides tonight after practice.");
assert(!Game.isBennettAsk(Game.workPlanFor(plannedFam, "chem-about-me")), "a plan is not an unanswered question");
const plannedAgain = Game.saveWorkPlan(plannedFam, weekById["chem-about-me"], "Ask Mr. X first period.");
assert.strictEqual((plannedAgain.notes || []).filter((n) => n.kind === "plan").length, 1, "saving a plan updates the same row");
assert.strictEqual(Game.workPlanFor(plannedAgain, "chem-about-me").text, "Ask Mr. X first period.");
const needsPlanHtml = Game.needsYouListHtml(week, proofNow, { family: plannedAgain });
assert(/data-plan-work="chem-about-me"/.test(needsPlanHtml) && />Plan</.test(needsPlanHtml), "Needs you shows Plan next to Edit");
assert(/>Done</.test(needsPlanHtml) && /data-needs-done="chem-about-me"/.test(needsPlanHtml), "Needs you shows Done on the row");
...
assert(/build:\s*180/.test(fs.readFileSync(path.join(root, "js/build.js"), "utf8")), "BW_BUILD should be 180");
assert(/2026-09-09T16:20:00-05:00/.test(fs.readFileSync(path.join(root, "js/build.js"), "utf8")), "BW_BUILD modified is 2026-09-09T16:20:00-05:00");
...
const storyViewPrev = localStorage.getItem("bw-site-view");
const storySessionPrev = localStorage.getItem("bw-session");
const storyTelPrev = localStorage.getItem("bw-telemetry");
localStorage.removeItem("bw-session");
localStorage.removeItem("bw-site-view");
localStorage.removeItem("bw-telemetry");
assert(!Game.storyAvailable(roster), "Me view without teammates still hides Story");
localStorage.setItem("bw-session", JSON.stringify({ user: "bennett", at: "2026-09-15T13:00:00.000Z" }));
assert(Game.storyAvailable(roster), "Bennett login opens Story with no teammates unlocked");
assert(Game.shouldRecordBennettLogin(), "Bennett login records a story day");
const storyChip = { hidden: true, id: "story-chip" };
const storyQsPrev = document.querySelectorAll;
document.querySelectorAll = (sel) => (sel && String(sel).indexOf("story-chip") >= 0 ? [storyChip] : []);
Game.paintStoryChip(roster);
assert.strictEqual(storyChip.hidden, false, "paintStoryChip unhides Story for Bennett login");
localStorage.removeItem("bw-session");
storyChip.hidden = true;
Game.paintStoryChip(roster);
assert.strictEqual(storyChip.hidden, true, "paintStoryChip hides Story again when Bennett is signed out");
document.querySelectorAll = storyQsPrev;
localStorage.setItem("bw-session", JSON.stringify({ user: "orin", at: "2026-09-15T13:00:00.000Z" }));
Game.setSiteView("bennett");
assert(Game.storyAvailable(roster), "Preview as Bennett still shows Story");
assert(!Game.shouldRecordBennettLogin(), "Orin preview as Bennett does not record a story day");
...
localStorage.removeItem("bw-session");
localStorage.removeItem("bw-site-view");
localStorage.setItem("bw-telemetry", JSON.stringify({ url: "https://example.supabase.co", anonKey: "anon", familyToken: "fam", role: "bennett" }));
Game.setSiteView("me");
assert(Game.storyAvailable(roster), "Bennett device shows Story even if the view was left on Me");
assert(Game.shouldRecordBennettLogin(), "Bennett device without a parent session records login days");
localStorage.setItem("bw-session", JSON.stringify({ user: "orin", at: "2026-09-15T13:00:00.000Z" }));
...
assert(!Game.shouldRecordBennettLogin(), "Orin on a Bennett-tagged phone does not advance the strip");
Game.logout();
if (storyTelPrev) localStorage.setItem("bw-telemetry", storyTelPrev);
else localStorage.removeItem("bw-telemetry");
if (storyViewPrev) localStorage.setItem("bw-site-view", storyViewPrev);
else localStorage.removeItem("bw-site-view");
if (storySessionPrev) localStorage.setItem("bw-session", storySessionPrev);
...
assert(/storyAvailable/.test(storyJs) && !/comicUnlocked\(roster\)/.test(storyJs), "Story page opens for Bennett without a teammate gate");
assert(!/Unlock three teammates/.test(storyHtml), "Story gate no longer tells Bennett to unlock three teammates");
assert(/storyAvailable/.test(fs.readFileSync(path.join(root, "js/game.js"), "utf8")), "HUD Story chip uses storyAvailable");
...
assert(Game.filterBennettHelp("story").some((row) => row.id === "story" && /one new page/i.test(row.body) && /pops up on This Week/i.test(row.body) && /signed in/i.test(row.body) && !/Coming soon/.test(row.body) && !/three teammates/i.test(row.body)), "Help says Story is on the bar when Bennett is signed in and celebrates a new page on This Week");
...
assert(/recordLoginDay[\s\S]{0,220}flushStoryPageCelebrate/.test(weekJs), "week boot flushes the story reward right after recordLoginDay");
...
const deviceTelPrev = localStorage.getItem("bw-telemetry");
const deviceViewPrev = localStorage.getItem("bw-site-view");
localStorage.removeItem("bw-session");
localStorage.setItem("bw-telemetry", JSON.stringify({ url: "https://example.supabase.co", anonKey: "anon", familyToken: "fam", role: "bennett" }));
localStorage.setItem("bw-site-view", "me");
assert.strictEqual(Game.recordStoryDay("2026-09-11").reached, 3, "Bennett device still unlocks a page when the view was left on Me");
assert.strictEqual(Game.recordStoryDay("2026-09-11").unlocked, false, "same Chicago day on the Bennett device does not unlock twice");
if (deviceTelPrev) localStorage.setItem("bw-telemetry", deviceTelPrev);
else localStorage.removeItem("bw-telemetry");
if (deviceViewPrev) localStorage.setItem("bw-site-view", deviceViewPrev);
else localStorage.removeItem("bw-site-view");
localStorage.setItem("bw-session", JSON.stringify({ user: "bennett", at: "2026-09-09T13:00:00.000Z" }));
...
assert.strictEqual(JSON.parse(localStorage.getItem("bw-story-unlock")).pendingCelebrate, 1, "pendingCelebrate is stored on bw-story-unlock");
...
assert.strictEqual(Game.getStoryUnlock().pendingCelebrate, 1, "showing the overlay does not clear pending until dismiss");
assert(Game.maybeCelebrateStoryPage(storyJson), "a reload before dismiss still shows the reward");
Game.markStoryCelebrateDismissed(1);
assert.strictEqual(Game.getStoryUnlock().pendingCelebrate, 0, "dismiss clears pendingCelebrate");
assert.strictEqual(Game.getStoryUnlock().celebratedReached, 1, "dismiss records celebratedReached");
assert.strictEqual(Game.recordStoryDay("2026-09-09").unlocked, false, "same Chicago day does not record another page");
assert.strictEqual(Game.maybeCelebrateStoryPage(storyJson), false, "after dismiss, same-day revisit does not celebrate");
assert.strictEqual(Game.recordStoryDay("2026-09-10").unlocked, true);
assert.strictEqual(Game.getStoryUnlock().pendingCelebrate, 2, "the next day stores pendingCelebrate for the new page");
...
Game.markStoryCelebrateDismissed(2);
localStorage.setItem("bw-story-unlock", JSON.stringify({ startYmd: "2026-09-09", lastYmd: "2026-09-14", reached: 6, celebratedReached: 6, pendingCelebrate: 0 }));
assert.strictEqual(Game.recordStoryDay("2026-09-15").reached, 7);
assert.strictEqual(Game.getStoryUnlock().pendingCelebrate, 7);
...
Game.markStoryCelebrateDismissed(7);
localStorage.setItem("bw-story-unlock", JSON.stringify({ startYmd: "2026-09-09", lastYmd: "2026-09-09", reached: 1 }));
assert.strictEqual(Game.pendingStoryCelebratePage(), 1, "an undismissed reached page is still pending after a reload");
assert(Game.maybeCelebrateStoryPage(storyJson), "backfill celebrates the undismissed page on This Week");
Game.markStoryCelebrateDismissed(1);
localStorage.setItem("bw-story-unlock", JSON.stringify({ startYmd: "2026-09-09", lastYmd: "2026-09-15", reached: 7, celebratedReached: 7, pendingCelebrate: 0 }));
assert.strictEqual(Game.recordStoryDay("2026-09-16").unlocked, true);
const pendingBeforePreview = Game.pendingStoryCelebratePage();
assert.ok(pendingBeforePreview > 0, "day 8 leaves a pending reward");
...
assert.strictEqual(Game.pendingStoryCelebratePage(), pendingBeforePreview, "parent preview does not consume Bennett's pending reward");
...
assert(!Game.alreadyUnlockedContent("ace-frog") && !Game.alreadyUnlockedContent("riff-bird") && !Game.alreadyUnlockedContent("scorch-spider") && !Game.alreadyUnlockedContent("crew-six-as-run"), "kid view + preview-all must not treat fight clips as earned");
...
assert(!/body\.site-view-kid \[data-add-item\]/.test(themeCss) && !/body\.site-view-kid \.class-card-tools/.test(themeCss), "kid CSS does not hide Progress Add assignment");
assert(/body\.site-view-kid \[data-edit\]/.test(themeCss) && /body\.site-view-kid \[data-del\]/.test(themeCss), "kid CSS still hides parent catalog Edit/Delete");
...
const fiveDay = (achievements.achievements || []).find((a) => a.id === "started-week-5");
assert(fiveDay && fiveDay.unlock && fiveDay.unlock.type === "login_days" && fiveDay.unlock.count === 5, "Five-day start auto-awards after 5 login days");
assert.strictEqual(fiveDay.icon, "clarinet", "Five-day start keeps the clarinet badge");
...
const nextLoginIntent = Game.parseAwardIntent("Bennett's next Jungle Jam login. Preview as Bennett does not count.");
assert.strictEqual(nextLoginIntent.type, "bennett_login");
assert(/Preview as Bennett does not count/i.test(nextLoginIntent.readout || nextLoginIntent.how || ""), "next-login readout says Preview does not count");
...
Game.touchWork("chem-open", "done");
assert(Game.openWorkTouched(openWeek), "marking the last item Done still earns open_touched");
const laterWeek = {
  work: openWeek.work.concat([
    { id: "later-book", title: "Book report", kind: "assignment", status: "open", due: "2026-10-18T23:59:00" }
  ])
};
assert(Game.openWorkTouched(laterWeek), "work due after This Week does not block Scorch");
["bw-unlocks", "bw-character-unlocks", "bw-family"].forEach((key) => localStorage.removeItem(key));
const scorchLive = (achievements.achievements || []).find((a) => a.id === "all-assignments-updated");
assert(scorchLive && scorchLive.unlock && scorchLive.unlock.type === "open_touched" && scorchLive.rewardCharacter === "scorch", "Scorch live unlock is every open assignment this week");
assert(!/all-assignments-updated/.test(gameJs.slice(gameJs.indexOf("const PREVIEW_AWARD_IDS"), gameJs.indexOf("const PREVIEW_CHARACTER_IDS"))), "live Scorch is not a preview-only trophy");
const scorchUnlock = Game.applyLiveUnlocks({ achievements: achievements.achievements }, Game.emptyFamily(), { week: laterWeek });
assert(Game.alreadyUnlocked("all-assignments-updated"), "updating every This Week assignment earns Meet Scorch");
...
["bw-unlocks", "bw-character-unlocks", "bw-content-unlocks", "bw-content-seen", "bw-family"].forEach((key) => localStorage.removeItem(key));
Game.setSessionUser("mom");
const momSix = Game.maybeAwardSixAs({ achievements: achievements.achievements }, Game.emptyFamily());
assert(!momSix.awarded && !momSix.celebrate, "Mom login does not grant Six A's rush");
Game.setSessionUser("orin");
Game.setSiteView("bennett");
const previewSix = Game.maybeAwardSixAs({ achievements: achievements.achievements }, Game.emptyFamily());
assert(!previewSix.awarded && !previewSix.celebrate, "Dad previewing Bennett does not grant Six A's rush");
["bw-unlocks", "bw-character-unlocks", "bw-content-unlocks", "bw-content-seen", "bw-family"].forEach((key) => localStorage.removeItem(key));
Game.setSessionUser("bennett");
const bennettSix = Game.maybeAwardSixAs({ achievements: achievements.achievements }, Game.emptyFamily());
assert(bennettSix.awarded && bennettSix.celebrate, "Bennett login grants Six A's rush");
assert.strictEqual(bennettSix.achievement && bennettSix.achievement.title, "6 A's in your classes!", "Six A's celebration title");
assert(Game.alreadyUnlocked("six-as-classes") && Game.alreadyUnlockedContent("crew-six-as-run"), "Bennett login unlocks Six A's rush");
assert(Game.canPlayLibraryItem(Game.libraryItem(seededLib, "crew-six-as-run")), "Six A's rush plays after the login award");
Game.markContentSeen("crew-six-as-run");
const bennettSixAgain = Game.maybeAwardSixAs({ achievements: achievements.achievements }, bennettSix.family);
assert(!bennettSixAgain.awarded && !bennettSixAgain.celebrate, "seen Six A's rush does not re-celebrate every login");
["bw-unlocks", "bw-character-unlocks", "bw-content-unlocks", "bw-content-seen", "bw-family", "bw-character-seen"].forEach((key) => localStorage.removeItem(key));
Game.setSessionUser("bennett");
const sixLogin = Game.playBennettLoginAwards({ achievements: achievements.achievements }, Game.emptyFamily(), seededLib, {});
assert(sixLogin.sixAs && sixLogin.sixAs.celebrate && sixLogin.sixAs.achievement && sixLogin.sixAs.achievement.title === "6 A's in your classes!", "playBennettLoginAwards prefers the Six A's popup over Scorch when both are fresh");
Game.setSessionUser("orin");
Game.setSiteView("me");
["bw-unlocks", "bw-character-unlocks", "bw-family", "bw-progress", "bw-character-seen"].forEach((key) => localStorage.removeItem(key));
...
assert.strictEqual(Game.consecutiveLoginStreak([Game.shiftChicagoYmd(todayYmd, -2), todayYmd], todayYmd), 1, "a gap resets the login streak");
["bw-unlocks", "bw-character-unlocks", "bw-login-days", "bw-family", "bw-session"].forEach((key) => localStorage.removeItem(key));
localStorage.setItem("bw-session", JSON.stringify({ user: "bennett", at: "2026-08-20T12:00:00.000Z" }));
const loginFam = Game.recordLoginDay(Game.emptyFamily());
assert(Game.getLoginDays(loginFam).indexOf(todayYmd) >= 0, "Bennett login stamps today's Chicago day");
loginFam.loginDays = fiveDays.slice();
assert(Game.evaluate({ unlock: { type: "login_days", count: 5 } }, { family: loginFam, ymd: todayYmd }), "login_days evaluate is true at 5");
["bw-unlocks", "bw-character-unlocks", "bw-character-seen", "bw-preview-all", "bw-preview-ids", "bw-family", "bw-login-days"].forEach((key) => localStorage.removeItem(key));
const fiveUnlock = Game.applyLiveUnlocks({ achievements: achievements.achievements }, Object.assign(Game.emptyFamily(), { loginDays: fiveDays }), { ymd: todayYmd });
...
assert(/id="give-picker"/.test(parentHtml) && /id="earn-picker"/.test(parentHtml), "Awards form is He gets / He earns it by");
assert(/Schedule reward/.test(parentHtml) && /Gets /.test(parentJs) && /Bennett does/.test(parentJs), "Parent desk cards show Gets and Bennett does");
assert(/id="award-test"/.test(parentHtml) && /id="award-extra-unlock"/.test(parentHtml), "Awards form still has Test and extra reward details");
assert.strictEqual(Game.earnRulePlain({ unlock: { type: "open_touched" }, rewardCharacter: "scorch" }), "Taps Done or I started this on every open assignment");
assert.strictEqual(Game.earnRulePlain({ title: "Meet Scorch" }), "You tap Award");
assert.strictEqual(Game.awardLiveStatus({ id: "x", unlock: { type: "open_touched" } }, { streaks: {} }).key, "live");
assert.strictEqual(Game.awardLiveStatus({ id: "x" }, { streaks: {} }).key, "parent");
...
assert(/\.quest-card/.test(themeCss) && /\.earn-pick\.on/.test(themeCss), "theme styles quest cards and earn chips");
...
Game.setSiteView("bennett");
const kidWorkEdit = Game.entryButtons("work:chem-aboutme-disc", "work:chem-aboutme-disc", { kidEdit: true });
assert(/data-edit-work="chem-aboutme-disc"/.test(kidWorkEdit), "Bennett Edit uses data-edit-work so kid CSS cannot hide it");
assert(!/\sdata-edit="/.test(kidWorkEdit), "Bennett assignment Edit is not the parent data-edit control");
assert(/Edit/.test(kidWorkEdit), "Bennett can edit an assignment");
...
assert(Game.filterBennettHelp("collapsed").some((row) => row.id === "progress" && /starts collapsed/.test(row.body)), "Help says By class starts collapsed");
assert(/classStartsOpen/.test(progressJs) && /rememberOpenClasses/.test(progressJs), "Progress By class starts collapsed and remembers a tap");
assert(!/: hasNeed/.test(progressJs), "Progress does not auto-expand every class with Needs you");
assert(Game.filterBennettHelp("needs you").some((row) => row.id === "needs-you" && /takes it off this list/.test(row.body)), "Help says Needs you Done clears the list");
assert(/closest\("\[data-needs-done\]"\)/.test(progressJs) && /closest\("\[data-needs-done\]"\)/.test(weekJs), "Needs you Done click is delegated so it still works after a redraw");
...
assert.strictEqual(Game.toLocalInput("2026-08-21"), "2026-08-21T23:59", "date-only due fills datetime-local");
assert.strictEqual(Game.toLocalInput("2026-08-21T13:05:00"), "2026-08-21T13:05", "datetime due keeps hours for the picker");
localStorage.removeItem("bw-family");
const progressSeed = {
  classes: [
    { id: "chemistry", name: "Chemistry", items: [{ id: "chem-lab", title: "Lab writeup", kind: "assignment" }] },
    { id: "english-10", name: "English 10", items: [] }
  ]
};
const editedWork = Game.updateAssignment(Game.emptyFamily(), progressSeed, "chem-lab", {
  title: "Lab rewrite",
  classId: "chemistry",
  due: "2026-08-25T23:59:00"
});
assert.strictEqual(editedWork.overlay.progress.itemEdits["chem-lab"].title, "Lab rewrite", "Progress overlay stores the new title");
assert(editedWork.overlay.progress.itemEdits["chem-lab"].updatedAt, "item edits stamp updatedAt so a pull cannot clobber a save");
assert.strictEqual(editedWork.overlay.week.edits.work["chem-lab"].due, "2026-08-25T23:59:00", "week overlay stores the new due");
const paintedWork = Game.applyProgressOverlay(progressSeed, editedWork);
assert.strictEqual(paintedWork.classes.find((cls) => cls.id === "chemistry").items[0].title, "Lab rewrite", "Progress paints the saved title");
assert.strictEqual(paintedWork.classes.find((cls) => cls.id === "chemistry").items[0].due, "2026-08-25T23:59:00", "Progress paints the saved due");
const movedWork = Game.updateAssignment(editedWork, progressSeed, "chem-lab", {
  title: "Lab rewrite",
  classId: "english-10",
  due: "2026-08-25T23:59:00"
});
const movedPaint = Game.applyProgressOverlay(progressSeed, movedWork);
assert(!movedPaint.classes.find((cls) => cls.id === "chemistry").items.some((item) => item.id === "chem-lab"), "moved assignment leaves the old class");
assert(movedPaint.classes.find((cls) => cls.id === "english-10").items.some((item) => item.id === "chem-lab"), "moved assignment lands in the new class");
localStorage.removeItem("bw-family");
...
assert(/checkin-group/.test(Game.checkinsListHtml(checkFam)), "Progress groups check-in answers by question");
...
assert.strictEqual(seededPool.reflections.pool.length, 15, "empty family gets the rotating check-in prompts");
assert.strictEqual(Game.DEFAULT_REFLECTION_POOL.length, 15, "fifteen check-in questions ship in the default pool");
assert(Game.DEFAULT_REFLECTION_POOL.some((p) => p.id === "r-fav-teacher") && Game.DEFAULT_REFLECTION_POOL.some((p) => p.id === "r-comic"), "pool covers favorite teacher and comic fuel");
const thinPool = Game.emptyFamily();
thinPool.reflections.pool = [
...
assert.strictEqual(Game.ensureReflectionPool(thinPool).reflections.pool.length, 15, "existing 3-prompt families pick up the new rotation");
assert(Game.todaysReflectionPrompt(Game.emptyFamily()).id, "today's prompt exists even if the overlay pool is empty");
localStorage.removeItem("bw-family");
assert(/Daily question/.test(Game.messagesInboxHtml(checkFam, { work: [], events: [] }, { canEdit: true, view: "me" })), "Mom and Dad see daily questions on Messages");
const goneCheck = Game.deleteAnswer(checkFam, checkFam.reflections.answers[0].id);
assert.strictEqual((goneCheck.reflections.answers || []).length, 0, "deleted check-in leaves the answers list");
assert(!((goneCheck.overlay.reflections && goneCheck.overlay.reflections.answers) || []).some((a) => a && a.text === "Chem lab"), "overlay does not resurrect a deleted check-in");
assert(!/Chem lab/.test(Game.checkinsListHtml(goneCheck)), "Progress check-ins drop a deleted answer");
assert(!/Chem lab/.test(Game.messagesInboxHtml(goneCheck, { work: [], events: [] }, { canEdit: true, view: "me" })), "Messages drop a deleted check-in");
const dayFam = Game.emptyFamily();
dayFam.notes = [
  { id: "q-today", from: "bennett", kind: "question", text: "today ask", at: "2026-08-20T10:00:00-05:00", targetType: "work", targetId: "w-today" },
  { id: "q-yday", from: "bennett", kind: "question", text: "older ask", at: "2026-08-19T17:53:00-05:00", targetType: "work", targetId: "w-yday" }
];
const dayHtml = Game.messagesInboxHtml(dayFam, { work: [], events: [] }, { canEdit: true, view: "me" });
assert(/msg-day/.test(dayHtml), "Messages groups by day");
assert(dayHtml.indexOf("today ask") < dayHtml.indexOf("older ask"), "newest day is on top");
assert(/body\.messages-page \.inbox-card/.test(themeCss), "Messages cards tighten on the messages page");
assert(!/Also rotating/.test(Game.checkinsListHtml(checkFam)), "Progress does not dump the idle question rotation");
assert(/messages\.html\?tab=daily/.test(progressJs) && /messages\.html\?tab=class/.test(progressJs), "Progress Check-ins links to Daily questions and Class messages");
assert(/data-parent-tab="daily"/.test(parentHtml) && /id="daily-answers"/.test(parentHtml), "Parent desk has a Daily questions tab");
assert(/data-hold-prompt/.test(fs.readFileSync(path.join(root, "js/parent.js"), "utf8")), "Parent Daily Qs can hold a prompt off rotation");
...
assert(!/if\s*\(\s*id\s*===\s*["']sociology["']\s*\)/.test(weekJs), "This Week does not special-case Sociology for grades");
assert(!/english-10/.test(JSON.stringify((progress.classes.find((cls) => cls.id === "english-10") || {}).grade || null)), "do not hardcode English into progress.json");
const chemDisc = week.work.find((w) => w.id === "chem-aboutme-disc");
assert(chemDisc && chemDisc.score === "1/1", "Chem discussion score is graded 1/1 on the week feed");
localStorage.removeItem("bw-ask-thread");
localStorage.removeItem("bw-family");
Game.saveAskThread({
  messages: [{
    id: "ask-comic-1",
    role: "bennett",
    title: "English 10: Finish summer comic strips",
    text: "my idea is to show me on a cruise with my mom",
    at: "2026-08-20T10:21:00-05:00"
  }]
});
const comicWeek = { work: [{ id: "eng-comics", title: "English 10: Summer Narrative Comic Strip", classId: "english-10" }] };
let comicFam = Game.promoteAskThreadToInbox(Game.emptyFamily(), comicWeek);
assert.strictEqual((comicFam.notes || []).length, 1, "comic Ask promotes once");
assert.strictEqual(comicFam.notes[0].targetId, "eng-comics", "comic Ask binds to the week assignment, not a help- slug");
assert(!/^help-/.test(String(comicFam.notes[0].id || "")), "comic Ask does not invent a help- id");
const comicId = comicFam.notes[0].id;
comicFam = Game.deleteNote(comicFam, comicId);
assert.strictEqual((comicFam.notes || []).length, 0, "deleted comic Ask leaves notes");
comicFam = Game.promoteAskThreadToInbox(comicFam, comicWeek);
assert.strictEqual((comicFam.notes || []).length, 0, "promote does not remint a deleted Ask");
comicFam.notes = Game.mergeNotesById(comicFam.notes, [{
  id: comicId,
  from: "bennett",
  kind: "question",
  text: "my idea is to show me on a cruise with my mom",
...
comicFam = Game.normalizeFamily(comicFam);
assert.strictEqual((comicFam.notes || []).length, 0, "note merge does not resurrect a deleted Ask");
assert(!/cruise with my mom/.test(Game.messagesInboxHtml(comicFam, comicWeek, { view: "me" })), "deleted Ask text is gone from Messages");
const packedGone = Telemetry.overlayToRow(comicFam.overlay, "fam");
assert(packedGone.week._jjDeletedNotes && packedGone.week._jjDeletedNotes.texts && packedGone.week._jjDeletedNotes.texts.length, "deleted Ask tombstones ride the family board");
assert.strictEqual(Telemetry.rowToOverlay(packedGone).deletedNotes.texts[0], "my idea is to show me on a cruise with my mom", "deleted Ask tombstones round-trip");
const mixFam = Game.emptyFamily();
mixFam.notes = [{ id: "q-slides", from: "bennett", kind: "question", text: "how many slides", at: "2026-08-20T10:21:00-05:00", targetType: "work", targetId: "eng-comics" }];
mixFam.reflections.answers = [{ id: "a-chem", prompt: "Which class felt easiest today?", text: "Chem lab", at: "2026-08-20T10:00:00-05:00" }];
assert(/how many slides/.test(Game.messagesInboxHtml(mixFam, comicWeek, { filter: "class", view: "me" })), "Class messages tab shows assignment threads");
assert(!/Chem lab/.test(Game.messagesInboxHtml(mixFam, comicWeek, { filter: "class", view: "me" })), "Class messages tab hides daily answers");
assert(/Chem lab/.test(Game.messagesInboxHtml(mixFam, comicWeek, { filter: "daily", view: "me" })), "Daily questions tab shows answers");
assert(!/how many slides/.test(Game.messagesInboxHtml(mixFam, comicWeek, { filter: "daily", view: "me" })), "Daily questions tab hides class threads");
let holdFam = Game.ensureReflectionPool(Game.emptyFamily());
holdFam.reflections.pool.forEach((p) => {
  holdFam = Game.setPromptPaused(holdFam, p.id, true);
});
assert.strictEqual(Game.todaysReflectionPrompt(holdFam), null, "held prompts leave the daily rotation");
localStorage.removeItem("bw-ask-thread");
localStorage.removeItem("bw-family");
...
const badgeFam = Game.emptyFamily();
badgeFam.streaks["desk-test"] = { awarded: true, awardedAt: "2026-08-20T21:43:00.000Z", preview: true };
Game.celebrate({
  id: "desk-test",
  title: "Logged in 5 days in a row",
  description: "Consistency is key! You've logged into the site 5 days in a row!",
  icon: "guitar"
}, { achievements: [] }, { items: [] }, { family: badgeFam });
const badgeLayer = document.body.children[document.body.children.length - 1];
assert(badgeLayer && /You unlocked this/.test(badgeLayer.innerHTML), "badge unlock shows the why screen");
assert(/See it in the Trophy room/.test(badgeLayer.innerHTML), "badge unlock asks Bennett to click into the Trophy room");
assert(/Consistency is key/.test(badgeLayer.innerHTML), "badge unlock shows what it was given for");
assert(/Earned /.test(badgeLayer.innerHTML), "badge unlock shows when");
...
assert(!/\smuted/.test(sixVidLayer.innerHTML.match(/<video[^>]*>/)[0]), "live Six A's rush video is unmuted");
assert(Game.playTrophyVideo(Game.libraryItem(Game.defaultLibrary(), "crew-six-as-clip"), { rewatch: true, preview: true }), "pedestal can replay the Six A's clip");
assert(/crew-six-as\.mp4/.test(document.body.children[document.body.children.length - 1].innerHTML), "pedestal Watch clip plays the 6s animation");
...
FakeAudio.plays.length = 0;
assert.strictEqual(Game.rewardMediaItem({ id: "six-as-classes", rewardMedia: "chunky", rewardUnlock: { type: "content", id: "crew-six-as-run" } }, chunkyLib).id, "dev-chunky", "Six A's sound is Chunky, not the rush video");
assert(Game.playAwardSound({ id: "six-as-classes", rewardMedia: "chunky", rewardUnlock: { type: "content", id: "crew-six-as-run" } }, Game.emptyFamily(), chunkyLib), "Six A's writeup plays Chunky");
assert(FakeAudio.plays.some((src) => /Chunky/i.test(src)), "Six A's why screen starts Chunky");
FakeAudio.plays.length = 0;
assert(Game.playAwardSound({ id: "six-as-classes" }, Game.emptyFamily(), chunkyLib), "Six A's still finds Chunky if the catalog row lost rewardMedia");
assert(FakeAudio.plays.some((src) => /Chunky/i.test(src)), "Six A's fallback still plays Chunky");
